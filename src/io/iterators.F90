...
#ifdef PART_FIELD
      CASE (c_dump_part_ex)  ! Z. Gong 
        DO WHILE (ASSOCIATED(cur) .AND. (part_count < npoint_it))
          part_count = part_count + 1 
          array(part_count) = cur%part_ex
          cur => cur%next
        END DO
      CASE (c_dump_part_ey)
        DO WHILE (ASSOCIATED(cur) .AND. (part_count < npoint_it))
          part_count = part_count + 1 
          array(part_count) = cur%part_ey
          cur => cur%next
        END DO
      CASE (c_dump_part_ez)
        DO WHILE (ASSOCIATED(cur) .AND. (part_count < npoint_it))
          part_count = part_count + 1 
          array(part_count) = cur%part_ez
          cur => cur%next
        END DO
     CASE (c_dump_part_bx)
        DO WHILE (ASSOCIATED(cur) .AND. (part_count < npoint_it))
          part_count = part_count + 1 
          array(part_count) = cur%part_bx
          cur => cur%next
        END DO
      CASE (c_dump_part_by)
        DO WHILE (ASSOCIATED(cur) .AND. (part_count < npoint_it))
          part_count = part_count + 1 
          array(part_count) = cur%part_by
          cur => cur%next
        END DO
      CASE (c_dump_part_bz)
        DO WHILE (ASSOCIATED(cur) .AND. (part_count < npoint_it))
          part_count = part_count + 1
          array(part_count) = cur%part_bz
          cur => cur%next
        END DO
#endif
      ...
      CASE (c_dump_part_opdepth)
        DO WHILE (ASSOCIATED(cur) .AND. (part_count < npoint_it))
          ...
          cur => cur%next
        END DO
	  CASE (c_dump_part_eta)   ! Gong
        DO WHILE (ASSOCIATED(cur) .AND. (part_count < npoint_it))
          part_count = part_count + 1
          array(part_count) = cur%particle_eta
          cur => cur%next
        END DO

      CASE (c_dump_part_radn)
        DO WHILE (ASSOCIATED(cur) .AND. (part_count < npoint_it))
          part_count = part_count + 1
          array(part_count) = cur%particle_radn
          cur => cur%next
        END DO

      CASE (c_dump_part_radt)
        DO WHILE (ASSOCIATED(cur) .AND. (part_count < npoint_it))
          part_count = part_count + 1
          array(part_count) = cur%particle_radt
          cur => cur%next
        END DO
