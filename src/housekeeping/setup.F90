...
CONTAINS
  ...
  SUBROUTINE restart_data(step)
    ...
    DO iblock = 1, nblocks
      ...
      CASE(c_blocktype_point_variable)
        ...
        ELSE IF (block_id(1:13) == 'particle_eta/') THEN ! Gong
#ifdef PHOTONS
          CALL sdf_read_point_variable(sdf_handle, npart_local, &
              species_subtypes(ispecies), it_particle_eta)
#else
          IF (rank == 0) THEN 
            PRINT*, '*** ERROR ***'
            PRINT*, 'Cannot load dump file with particle eta.'
            PRINT*, 'Please recompile with the -DPHOTONS option.'
          END IF
          CALL abort_code(c_err_pp_options_missing)
          STOP 
#endif
        ELSE IF (block_id(1:14) == 'particle_radn/') THEN 
#ifdef PHOTONS
          CALL sdf_read_point_variable(sdf_handle, npart_local, &
              species_subtypes(ispecies), it_particle_radn)
#else
          IF (rank == 0) THEN
            PRINT*, '*** ERROR ***'
            PRINT*, 'Cannot load dump file with particle radn.'
            PRINT*, 'Please recompile with the -DPHOTONS option.'
          END IF
          CALL abort_code(c_err_pp_options_missing)
          STOP
#endif
        ELSE IF (block_id(1:14) == 'particle_radt/') THEN
#ifdef PHOTONS
          CALL sdf_read_point_variable(sdf_handle, npart_local, &
              species_subtypes(ispecies), it_particle_radt)
#else
          IF (rank == 0) THEN
            PRINT*, '*** ERROR ***'
            PRINT*, 'Cannot load dump file with particle radt.'
            PRINT*, 'Please recompile with the -DPHOTONS option.'
          END IF
          CALL abort_code(c_err_pp_options_missing)
          STOP
  ...
  FUNCTION it_particle_eta(array, npart_this_it, start, param)  ! Gong
    REAL(num) :: it_particle_eta
    REAL(num), DIMENSION(:), INTENT(IN) :: array
    INTEGER, INTENT(INOUT) :: npart_this_it
    LOGICAL, INTENT(IN) :: start
    INTEGER, INTENT(IN), OPTIONAL :: param
    INTEGER :: ipart
    DO ipart = 1, npart_this_it
      iterator_list%particle_eta = array(ipart)
      iterator_list => iterator_list%next
    END DO
    it_particle_eta = 0
  END FUNCTION it_particle_eta


  FUNCTION it_particle_radn(array, npart_this_it, start, param)
    REAL(num) :: it_particle_radn
    REAL(num), DIMENSION(:), INTENT(IN) :: array
    INTEGER, INTENT(INOUT) :: npart_this_it
    LOGICAL, INTENT(IN) :: start
    INTEGER, INTENT(IN), OPTIONAL :: param
    INTEGER :: ipart
    DO ipart = 1, npart_this_it
      iterator_list%particle_radn = array(ipart)
      iterator_list => iterator_list%next
    END DO
    it_particle_radn = 0
  END FUNCTION it_particle_radn

   FUNCTION it_particle_radt(array, npart_this_it, start, param)
    REAL(num) :: it_particle_radt
    REAL(num), DIMENSION(:), INTENT(IN) :: array
    INTEGER, INTENT(INOUT) :: npart_this_it
    LOGICAL, INTENT(IN) :: start
    INTEGER, INTENT(IN), OPTIONAL :: param
    INTEGER :: ipart
    DO ipart = 1, npart_this_it
      iterator_list%particle_radt = array(ipart)
      iterator_list => iterator_list%next
    END DO
    it_particle_radt = 0
  END FUNCTION it_particle_radt
