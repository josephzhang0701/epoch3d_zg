...
CONTAINS
  ...
  SUBROUTINE pack_particle(array, a_particle)
    ...
    array(cpos+1) = a_particle%particle_eta ! Z. Gong
    array(cpos+2) = a_particle%particle_radt
    array(cpos+3) = a_particle%particle_radn
    cpos = cpos+4
#endif
#if defined(PHOTONS) || defined(BREMSSTRAHLUNG)
...
#endif
#ifdef PART_FIELD
      array(cpos)   = a_particle%part_ex ! Z. Gong
     array(cpos+1) = a_particle%part_ey ! Z. Gong
   array(cpos+2) = a_particle%part_ez ! Z. Gong
    array(cpos+3) = a_particle%part_bx ! Z. Gong
    array(cpos+4) = a_particle%part_by ! Z. Gong
    array(cpos+5) = a_particle%part_bz ! Z. Gong
    cpos = cpos + 6
#endif
#ifdef WORK_DONE_INTEGRATED
  ...
  SUBROUTINE unpack_particle(array, a_particle)
    ...
    a_particle%particle_eta = array(cpos+1) ! Gong
     a_particle%particle_radt = array(cpos+2)
     a_particle%particle_radn = array(cpos+3)
     cpos = cpos+4
#endif
#if defined(PHOTONS) || defined(BREMSSTRAHLUNG)
...
#endif
#ifdef PART_FIELD
     a_particle%part_ex = array(cpos)   ! Z. Gong
     a_particle%part_ey = array(cpos+1)
     a_particle%part_ez = array(cpos+2)
     a_particle%part_bx = array(cpos+3)
     a_particle%part_by = array(cpos+4)
     a_particle%part_bz = array(cpos+5)
     cpos = cpos+6
#endif
#ifdef WORK_DONE_INTEGRATED
  ...
  SUBROUTINE init_particle(new_particle)
   ...
   new_particle%particle_eta = 0.0_num !Gong
      new_particle%particle_radn = 0.0_num !Gong
    new_particle%particle_radt = 0.0_num !Gong
...
#endif
#ifdef PART_FIELD
     new_particle%part_ex = 0_num ! Gong
     new_particle%part_ey = 0_num ! Gong
     new_particle%part_ez = 0_num ! Gong
     new_particle%part_bx = 0_num ! Gong
     new_particle%part_by = 0_num ! Gong
     new_particle%part_bz = 0_num ! Gong
#endif
#ifdef WORK_DONE_INTEGRATED
