...
CONTAINS
  ...
  FUNCTION io_block_handle_element(element, value) RESULT(errcode)
    ...
    ELSE IF (str_cmp(element, 'particle_eta')) THEN ! Gong
      elementselected = c_dump_part_eta
    ELSE IF (str_cmp(element, 'particle_radn')) THEN
      elementselected = c_dump_part_radn
    ELSE IF (str_cmp(element, 'particle_radt')) THEN
      elementselected = c_dump_part_radt
#endif
...
#ifdef PART_FIELD
    ELSE IF (str_cmp(element, 'part_ex')) THEN  ! Gong
      elementselected = c_dump_part_ex
    ELSE IF (str_cmp(element, 'part_ey')) THEN  ! Gong
      elementselected = c_dump_part_ey
    ELSE IF (str_cmp(element, 'part_ez')) THEN  ! Gong
      elementselected = c_dump_part_ez
    ELSE IF (str_cmp(element, 'part_bx')) THEN  ! Gong
      elementselected = c_dump_part_bx
    ELSE IF (str_cmp(element, 'part_by')) THEN  ! Gong
      elementselected = c_dump_part_by
    ELSE IF (str_cmp(element, 'part_bz')) THEN  ! Gong
      elementselected = c_dump_part_bz
#endif
...
#define PARTICLE_ID
  ...
  SUBROUTINE set_restart_dumpmasks
    ...
    io_block%dumpmask(c_dump_part_eta) = &   ! Gong
        IOR(io_block%dumpmask(c_dump_part_eta), c_io_restartable)
    io_block%dumpmask(c_dump_part_radn) = &
        IOR(io_block%dumpmask(c_dump_part_radn), c_io_restartable)
    io_block%dumpmask(c_dump_part_radt) = &
        IOR(io_block%dumpmask(c_dump_part_radt), c_io_restartable)
#endif
#if defined(PHOTONS) || defined(BREMSSTRAHLUNG)
...
#endif
#ifdef PART_FIELD
    io_block%dumpmask(c_dump_part_ex) = &  ! Z. Gong
         IOR(io_block%dumpmask(c_dump_part_ex), c_io_restartable)
    io_block%dumpmask(c_dump_part_ey) = &
         IOR(io_block%dumpmask(c_dump_part_ey), c_io_restartable)
    io_block%dumpmask(c_dump_part_ez) = &
         IOR(io_block%dumpmask(c_dump_part_ez), c_io_restartable)
    io_block%dumpmask(c_dump_part_bx) = &
         IOR(io_block%dumpmask(c_dump_part_bx), c_io_restartable)
    io_block%dumpmask(c_dump_part_by) = &
         IOR(io_block%dumpmask(c_dump_part_by), c_io_restartable)
    io_block%dumpmask(c_dump_part_bz) = &
         IOR(io_block%dumpmask(c_dump_part_bz), c_io_restartable)
#endif
#ifdef WORK_DONE_INTEGRATED
